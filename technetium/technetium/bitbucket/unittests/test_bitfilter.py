...
class BitfilterTest(unittest.TestCase):
    ...
    def test_filter_issues_by_combination(self):
        ...
        if datetime.datetime.strftime(datetime.date.today(),'%m') == '12':
            filter_month_value = 'last_month'
        self.assertEqual(bitfilter.filter_issues({'type':'task', 'priority':'major', 'status':'new', 'assignee':'Albert Chieu', 'date':filter_month_value}, self.all_parsed_issues), expected_result_issue)        
